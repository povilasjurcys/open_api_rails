...
RSpec.describe OpenApiRails::Schema::Adapters::SimpleSchemaSerializer do
  subject(:simple_schema_serializer) { described_class.new(schema) }

  let(:schema) { instance_double(OpenApiRails::Schema, paths: schema_paths) }
  let(:schema_paths) { {} }

  describe '#paths' do
    subject(:paths) { simple_schema_serializer.paths }
